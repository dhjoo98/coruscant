...
__device__ __forceinline__ void Coruscant_STC_CopyTileFromGlobalToShared_X_64(half* __restrict__ SharedPTR,
                                                                ...
                                                                bool        Pred = true) //to stop copying at the end of pipeline
{
    ...
    //But this is just writing to the correct. no actual swizzling, just having each thread load different elements to the correct place 
    //still col-major thou. 
...
#pragma unroll
    for (int i = 0; i < MaxIteration; i++) {
        ...
        // AsyncCopyPredictor );
        //col-major loading to shared memory, that is, K is the leading dimension. 
    }
}

template<typename TilingConfig>
__device__ __forceinline__ void Coruscant_STC_PipelinedCoreComputations(float c[][REG_PER_C_TENSOR_16_16],
                                                          ...
                                                          int warp_start_col)
{
    ...
    Coruscant_STC_FragLoadFromSharedToRegisters<WARP_ROW_TENSORS>(a, SharedMemoryPTR, warp_start_row, 0); //defined in MMA_PTX.cuh
    Coruscant_STC_B_FragLoadFromSharedToRegisters<TilingConfig::WARP_COL_TENSORS, TilingConfig::N8>(
...
#pragma unroll
    for (int k = 0; k < BLOCK_K_TENSORS; k++) { //BLOCK_K_TENSOR also defined in TilingConfig. 
        ...
        if (k + 1 < BLOCK_K_TENSORS) {
            Coruscant_STC_FragLoadFromSharedToRegisters<WARP_ROW_TENSORS>(a_write, SharedMemoryPTR, warp_start_row, (k + 1) * MMA_K);
            Coruscant_STC_B_FragLoadFromSharedToRegisters<TilingConfig::WARP_COL_TENSORS, TilingConfig::N8>(
...
#pragma unroll
            for (int j = 0; j < TilingConfig::WARP_COL_TENSORS; j++) { //4
...
Coruscant_STC_StoreToSharedMemoryFromRegister(float (*smem_CFrag)[TilingConfig::TILE_M + PADDING_SHARED_MEM_FOR_C],
