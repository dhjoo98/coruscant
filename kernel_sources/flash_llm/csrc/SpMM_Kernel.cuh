...
__device__ __forceinline__ void FlashLLM_SpMM_CopyFromGlobalToReg(uint32_t*    Registers_GlobalToShared1,
...
template<typename TilingConfig, typename SparseKernelConfig>
__device__ __forceinline__ void FlashLLM_SpMM_CopyFromGlobalToShared(int          tid,
                                                            half*        smem,
...
__device__ __forceinline__ void FlashLLM_SpMM_InitSharedMemory(half* __restrict__ SharedPTR)
{
...
template<typename TilingConfig, typename SparseKernelConfig>
__device__ __forceinline__ void FlashLLM_SpMM_DecompressFromRegisterToShared(half* __restrict__ SharedPTR1,
...
template<typename TilingConfig, typename SparseKernelConfig>
__global__ void FlashLLM_SpMM_Kernel(const half*  A,
                            ...
                            int          Split_K)
{
    ...
    //
    FlashLLM_SpMM_CopyFromGlobalToReg<TilingConfig, SparseKernelConfig>(Registers_GlobalToShared,
                                                               &NNZ_ThreadLocal1,
                                                               ...
                                                               NNZ_ThisTile2);
    FlashLLM_SpMM_InitSharedMemory<TilingConfig>(smem);
    cp_async_group_commit();
    FlashLLM_CopyTileFromGlobalToShared_X_64<TilingConfig::TILE_N2, TilingConfig>(
    ...
    __syncthreads();
    FlashLLM_SpMM_DecompressFromRegisterToShared<TilingConfig, SparseKernelConfig>(
        smem,
        Registers_GlobalToShared,
        NNZ_ThreadLocal1,
...
#pragma unroll(1)
    for (int tile_id_k = 0; tile_id_k < NumIter; tile_id_k++) {
        ...
        FlashLLM_SpMM_InitSharedMemory<TilingConfig>(smem_write_PTR);
        cp_async_group_commit();
        FlashLLM_SpMM_CopyFromGlobalToReg<TilingConfig, SparseKernelConfig>(
            Registers_GlobalToShared,
            &NNZ_ThreadLocal1,
        ...
        FlashLLM_PipelinedCoreComputations<TilingConfig>(c, a, b, smem_read_PTR, warp_start_row, warp_start_col);
        //
    ...
    FlashLLM_StoreToSharedMemoryFromRegister<TilingConfig>(smem_CFrag, c);
    __syncthreads();
                                                         ...
                                                         int          NNZ_VECTOR_ThisTile2)
{
    ...
    *NNZ_VECTOR_ThreadLocal1 = Num_NNZ_Vector1;
    ...
    int Max_NNZ_VECTOR_ThisTile =
        (TilingConfig::TILE_M == 256) ? max(NNZ_VECTOR_ThisTile1, NNZ_VECTOR_ThisTile2) : NNZ_VECTOR_ThisTile1;
#pragma unroll
    for (int i = 0; i < SparseKernelConfig::NUM_REG_FOR_SPARSE_KERNEL / SparseKernelConfig::VECTOR_SIZE; i++) { //64/4=16 (validated from trace)
        int index = threadIdx.x + i * (WARP_SIZE * (TilingConfig::BLOCK_WARPS));
        if (index >= Max_NNZ_VECTOR_ThisTile)
            break;
        if (index < NNZ_VECTOR_ThisTile1
...
{
    //128 threads.
    int lane_id = threadIdx.x % WARP_SIZE; //id withn warp 0~31
    int warp_id = threadIdx.x / WARP_SIZE; //warp id 0~3
    //
    static_assert(TilingConfig::TILE_M % TilingConfig::BLOCK_WARPS == 0,
                  "TILE_M must be an integer multiple to BLOCK_WARPS");
    constexpr int RowsPerWarp = TilingConfig::TILE_M / TilingConfig::BLOCK_WARPS; //64 rows per warp.
    //
    static_assert(TILE_K == 64, "For now, TILE_K is assumed to be 64.\n");
    const int StartRowNum         = warp_id * RowsPerWarp;
    half*     SharedPTR_PerThread = SharedPTR + StartRowNum * TILE_K + HALF_PER_128B * lane_id;
...
#pragma unroll
    ...
    //placement considered, and written on Notion
        //smem divided into two, thus two 128x64s are sequentially placed. 
}
                            ...
                            int          Split_K)
{
    ...
    if (IsLastBatch)
        ...
        NumIter = AverageNumKBlock;
    //
    const int* TileOffsets_ThisBlock1 = nullptr; //"TileOffsets" are all pointers
    const int* TileOffsets_ThisBlock2 = nullptr;
    if (TilingConfig::TILE_M == 256) {
        //no different than mind. TileOffset is a block-wise variable. Two downward-consecutive 128x64 tile.
            //(K_Global / TILE_K)~num of tiles row-wise x (y*2): dual iterating two 128x64 tiles.
        TileOffsets_ThisBlock1 =
            TileOffsets + K_Global / TILE_K * y * 2 //K_Global / TILE_K : tiling the K dimension of Matrix A. 
            + BatchID * AverageNumKBlock;  // Address for matrix A, taking SplitK into consideration
        
        //if (threadIdx.x == 0 && blockIdx.y == 0)
        //    printf("K_Global %d, Tile_K %d \n", K_Global, TILE_K); 256 and 64 respectively ~ 4 
    ...
    //
    uint32_t Registers_GlobalToShared[SparseKernelConfig::NUM_REG_FOR_SPARSE_KERNEL]; //64
    uint32_t NNZ_ThreadLocal1 = 0;
    uint32_t NNZ_ThreadLocal2 = 0;
    //
    extern __shared__ __align__(128) half smem[];  // at least be 128 Bytes aligned
    ...
    uint32_t __restrict__ a[WARP_ROW_TENSORS * 2][4];
    uint32_t __restrict__ b[TilingConfig::WARP_COL_TENSORS * 2][4];
    // copying B tile from GlobalMemory to 
    const half* BTileGlobalPTR =
        B + Tile_Start_N * K_Global
        + BatchID * AverageNumKBlock * TILE_K;  // Address for matrix B, taking SplitK into consideration
    //
    ...
    SpMM_DecompressFromRegisterToShared<TilingConfig, SparseKernelConfig>(
        ...
        Registers_GlobalToShared,
        NNZ_ThreadLocal1,
        smem + TilingConfig::TILE_M * TILE_K / 2, //smem + 256*64/2 : see, conseqtive placement
        Registers_GlobalToShared + SparseKernelConfig::NUM_REG_FOR_SPARSE_KERNEL / 2,
...
#pragma unroll(1)
    for (int tile_id_k = 0; tile_id_k < NumIter; tile_id_k++) {
        ...
        BTileGlobalPTR = B + Tile_Start_N * K_Global + BatchID * AverageNumKBlock * TILE_K + ((tile_id_k + 1) * TILE_K);
        // double buffer
            //decompress to shared, //feed to TC from shared.
        half* __restrict__ smem_write_PTR = smem;
        half* __restrict__ smem_read_PTR  = smem;
        smem_write_PTR = smem + ((tile_id_k + 1) % 2) * (TilingConfig::TILE_M * TILE_K + TILE_K * TilingConfig::TILE_N);
            //(M*K + K*N) = (256*64 + 64(8)) A and B, smem is declared half.
        smem_read_PTR  = smem + ((tile_id_k) % 2) * (TilingConfig::TILE_M * TILE_K + TILE_K * TilingConfig::TILE_N);
        //
        bool GlobalCopy = (tile_id_k + 1) < NumIter;
