...
//donghyeon note: the extraction part must've loaded the amount of data used for all these loops.
    //outer loop k(BLOCK_K_TENSORS=4), for each tile in K-dimension, load the fragments. 
    //inner loop i(WARP_ROW_TENSORS=4), j(WARP_COL_TENSORS=4) for each tile in M and N dimension, compute the matrix multiplication.
